...
class model extends database{
	...
	function getCustomer_withAccountID($MATK){
		$sql = "SELECT TENKH, EMAIL_KH, SDT_KH, DIACHI_KH FROM khachhang, taikhoan WHERE khachhang.MAKH = taikhoan.MAKH AND taikhoan.MATK = '$MATK'";
		$this->setQuery($sql);
		return $this->loadAllRows();
	}
	...
	public function getRevenue($MONTH){
		$sql = "SELECT MADT, TONGCONG, NGAYLAP, TENNV, NGAYSINH_NV, DIACHI_NV, EMAIL_NV, SDT_NV, CHUCVU FROM doanhthu dt, nhanvien nv WHERE dt.MANV = nv.MANV AND THOIGIAN='$MONTH'";
		$this->setQuery($sql);
		$arr = $this->loadAllRows();
		for($i=0; $i<count($arr); $i++){
